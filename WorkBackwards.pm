...
sub page_o_problems{
  my $self = shift;
  my $n_problems = shift || 3;	# number of problems on the page.
  my $problems_string = '';
  for (1..$n_problems) {
    my ($problem_text, $answer_text) = $self->random_problem();
    $problems_string .= ' \item ' . "$problem_text " . "\n" . 
      answer_box('1.2in') . ' \newline  \vspace {5 mm} ' . "\n";
    $problems_string .= 'Show your work by filling in the numbers in this chain of boxes: \newline ' . "\n";
    $problems_string .= box_chain(4, 10, 8, $self->result() ) . "\n";
  }
  my $page_string = $self->page_tex_template();
$page_string =~ s{PAGE_TITLE}{Work Backwards};
